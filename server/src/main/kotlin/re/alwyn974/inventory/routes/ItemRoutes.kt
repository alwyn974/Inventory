    ...
    authenticate("jwt") {
        route("/items") {
            ...
            get("/{id}") {
                ...
                val itemId = call.parameters["id"] ?: return@get call.respond(HttpStatusCode.BadRequest)
                ...
                val itemDto = transaction {
                    val category = item[Items.category]?.let { categoryId ->
                        Categories.selectAll().where { Categories.id eq categoryId }.singleOrNull()?.let { categoryRow ->
                            CategoryDto(
                                id = categoryRow[Categories.id].toString(),
                                name = categoryRow[Categories.name],
                    ...
                    val folder = item[Items.folder]?.let { folderId ->
                    ...
                    val tags = ItemTags.join(Tags, JoinType.INNER, ItemTags.tag, Tags.id)
                        .selectAll().where { ItemTags.item eq item[Items.id] }
                        .map { tagRow ->
                            TagDto(
                                id = tagRow[Tags.id].toString(),
                                name = tagRow[Tags.name],
                    ...
                    ItemDto(
                        id = item[Items.id].toString(),
                        name = item[Items.name],
                        description = item[Items.description],
                        quantity = item[Items.quantity],
                        minQuantity = item[Items.minQuantity],
                        imageUrl = item[Items.imageUrl],
                        category = category,
                        folder = folder,
                        tags = tags,
                        createdBy = item[Items.createdBy].toString(),
                        createdAt = item[Items.createdAt].toString(),
                        updatedAt = item[Items.updatedAt].toString()
                    )
                }
            ...
            put("/{id}") {
                ...
                val itemId = call.parameters["id"] ?: return@put call.respond(HttpStatusCode.BadRequest)
            ...
            delete("/{id}") {
                ...
                val itemId = call.parameters["id"] ?: return@delete call.respond(HttpStatusCode.BadRequest)
            ...
            post("/{id}/image") {
                ...
                val itemId = call.parameters["id"] ?: return@post call.respond(HttpStatusCode.BadRequest)
